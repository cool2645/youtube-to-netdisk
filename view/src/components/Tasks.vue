      ...
      <div v-masonry-tile class="task-card-wrapper col col-md-3 col-sm-6 col-xs-12" v-for="(task, index) in data">
        <div class="task-card" :class="classObject(task)">
          <div id="brief" v-show="isLoaded && !task.showMore">
            ...
            <a target="_blank" v-if="task.file_name" :href="`/static/${task.youtube_id}/${encodeURIComponent(task.file_name)}`">本地下载</a>
            <a target="_blank" class="v-for" v-if="task.subtitles"
               v-for="sub in getSubtitles(task.subtitles)"
               :href="`/static/${task.youtube_id}/${encodeURIComponent(getSubtitleFilename(sub, task.file_name))}`"
